...
SELECT * FROM (
  SELECT DISTINCT ON (draft.article_id) article.id,
    draft.article_id, draft_id, draft.title AS title_orig, draft.title AS title, -- to be processed by opencc in JS
...
ALTER TABLE search_index.article ADD PRIMARY KEY (id) ;
...
CREATE INDEX search_index_title_fulltext_idx ON search_index.article USING GIN (to_tsvector('chinese_zh', title)) ;
CREATE INDEX search_index_text_content_fulltext_idx ON search_index.article USING GIN (to_tsvector('chinese_zh', text_content_converted)) ;
CREATE INDEX search_index_title_ts_gin_idx ON search_index.article USING GIN (title_ts) ;
CREATE INDEX search_index_text_ts_gin_idx ON search_index.article USING GIN (text_ts) ;
